...
$stmt = $conn->prepare("SELECT user_id, user_first_name, user_last_name, user_pass FROM users WHERE user_email = ?");
...
// Check if user found
if ($stmt->num_rows > 0) {
    $stmt->bind_result($user_id, $user_first, $user_last, $db_pass);
    $stmt->fetch();
